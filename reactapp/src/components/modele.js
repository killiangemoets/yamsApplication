...
export const createGameBoards = function () {
...
export const setNumPlayers = function (numPlayers) {
  state.numPlayers = numPlayers;
  state.scores = Array(numPlayers).fill(0);
};
...
export const updateGameBoard = function () {
  ...
  if (!state.gameBoards[state.currentPlayer].bonus.alreadyPlayed) {
    const numbersScore =
      state.gameBoards[state.currentPlayer].one.score +
      state.gameBoards[state.currentPlayer].two.score +
      state.gameBoards[state.currentPlayer].three.score +
      state.gameBoards[state.currentPlayer].four.score +
      state.gameBoards[state.currentPlayer].five.score +
      state.gameBoards[state.currentPlayer].six.score;

    state.gameBoards[state.currentPlayer].bonus.score = numbersScore;

    if (numbersScore >= scoreForBonus) {
      state.gameBoards[state.currentPlayer].bonus = {
        alreadyPlayed: true,
        score: scoreForBonus,
      };

      state.scores[state.currentPlayer] += bonus;
    }
  }
};
...
const calculateThreeFourYahtzeeScores = function () {
  ...
  if (eachNumber.find((el) => el >= 3))
    threeFourYahtzeeOrFullhouse.three = state.currentDices.reduce(
      (partialSum, a) => partialSum + a,
      0
    );
  if (eachNumber.find((el) => el >= 4))
    threeFourYahtzeeOrFullhouse.four = state.currentDices.reduce(
      (partialSum, a) => partialSum + a,
      0
    );
